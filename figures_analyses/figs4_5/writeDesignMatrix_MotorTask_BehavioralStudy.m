...
load('AllResults-Motor_localizer.mat');
...
for s=1:Subjects
   fprintf('Subject %d\n',s);
    ...
    DesignMatrixMotor = [DesignMatrixMotor; SID obs_num block_num a b X_allocation Y_allocation ...
...
num_col = size(DesignMatrixMotor,2);

%% Normalize
 for col=11:num_col % z_score mouse features
  DesignMatrixMotor(:,col)=(DesignMatrixMotor(:,col)-nanmean(DesignMatrixMotor(:,col))) / nanstd(DesignMatrixMotor(:,col));
end

%% Clean
num_to_round = (length(find(DesignMatrixMotor(:,11:end)>5 | DesignMatrixMotor(:,11:end)<-5))./numel(DesignMatrixMotor(:,11:end)))*100;
disp(['Data rounded: ' num2str(num_to_round) '%']);
for col=11:num_col
%     [B,TF] = rmoutliers(data2(:,col),'quartiles','ThresholdFactor',4);
%     data2(TF,col)=NaN;
    DesignMatrixMotor((DesignMatrixMotor(:,col))>5,col)=5;
    DesignMatrixMotor((DesignMatrixMotor(:,col))<-5,col)=-5;
...
samp_nans=[];
for col=11:num_col
    allnans(col)=mean(isnan(DesignMatrixMotor(:,col)));
    samp_nans=[samp_nans isnan(DesignMatrixMotor(:,col))];
end



%% Output
DesignMatrix_table = array2table(DesignMatrixMotor,'VariableNames',...
...
file_name = ['DesignMatrix_ZScoredAcrossSubject_MotorTask_BehavioralStudy.xls'];
