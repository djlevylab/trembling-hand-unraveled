...
%                       7: outlier marker
%                       8: boxfill (if used)
%       H_TICKLABELS graphics handle output to ticklabels in multigroup case 
%
...
%
% Example : 3 single data set with 4 columns, using boxplotCsub to colorize
...
%
% Example : 4 sets divided into 3 groups of data (showing default settings usage)
%   mydataset = randn(10,2,2);
%   boxplotCsub(mydataset(:,:,1),1,[],[],[],[],[],[],[],[1 2]);
%   boxplotCsub(mydataset(:,:,2),1,[],[],[],[],[],[],[],[2 2]);
%
...
%                   h(9,:)
%   BFGK 31-aug-14
%       Corrected "h_ticklabels" output for the case of single simple group
%       variables to be consistent. Before, was only called for multiline group
%       variable labels and gave an error.
...
if (~isempty(g))
   for i=1:n
      ...
      ht = 'Warning: H_TICKLABELS only available for multigroup groupings. Use classic get/set to modify labels.';
   end
